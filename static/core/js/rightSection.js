...
(function() {
  var CommentsItem, CommentsItemCommentLi, EditPage, ReplyItem, RightSectionPage, SingleCommentPage, Utils, _ref,
  ...
  SingleCommentPage = (function(_super) {
    ...
    SingleCommentPage.prototype.init = function(commentData, commentsItem) {
      ...
      this.UI['go-to-editer'].onclick = function() {
        var content;
        content = _this.UI['add-reply-content'].value;
        return _this.addReply(null, content);
      };
      this.UI['add-reply-confirm'].onclick = function() {
        var content;
        content = _this.UI['add-reply-content'].value;
        ...
        return _this.api.addReply(_this.commentData.commentid, content, function() {
          _this.UI['add-reply-content'].value = "";
      ...
      return this.api.getReplys(this.commentData.commentid, function(res) {
        ...
        for (_j = 0, _len1 = _ref2.length; _j < _len1; _j++) {
          ...
          item.on("replyToThis", function(replyItems) {
            _this.UI['add-reply-content'].focus();
            return _this.UI['add-reply-content'].J.text("@" + replyItems.data.nickname + " ");
          });
    ...
    SingleCommentPage.prototype.addReply = function(target, content) {
      var _this = this;
      if (content == null) {
        content = null;
      }
      console.log("add reply");
      return this.rightSection.showEditPage("addReply", content, function(content) {
        return _this.api.addReply(_this.commentData.commentid, content, function() {
          window.showMessage("Your reply has been added successfully.");
          return _this.initReplys(function() {
            return _this.commentsItem.updateReplyNum(_this.commentData.commentid, _this.replyItems.length);
  ...
  window.RightSection = (function(_super) {
    ...
    RightSection.prototype.initComments = function() {
      ...
      this.commentsItems = [];
      this.app.marks.sort(function(a, b) {
        return a.pageid - b.pageid || a.marky - b.marky;
      });
    ...
    RightSection.prototype.addComment = function(commentsItem, content) {
      var markData,
        _this = this;
      markData = commentsItem.markData;
      return this.showEditPage("addComment", content, function(content) {
        var markid;
  ...
  CommentsItem = (function(_super) {
    ...
    function CommentsItem(rightSection, comments, markData) {
      ...
      this.rightSection = rightSection;
      this.app = this.rightSection.app;
      this.api = this.rightSection.api;
      this.id = markData.markid;
      this.dom.id = "comments-" + this.id;
    ...
    CommentsItem.prototype.initBtns = function() {
      ...
      this.UI['add-comment'].onclick = function() {
        _this.UI['add-comment-box'].J.toggleClass("hide");
        if (!_this.UI['add-comment-box'].J.hasClass("hide")) {
          return _this.UI['add-comment-content'].focus();
        }
      };
      this.UI['go-to-editer'].onclick = function() {
        var content;
        content = _this.UI['add-comment-content'].value;
        return _this.rightSection.addComment(_this, content);
      };
      this.UI['add-comment-cancel'].onclick = function() {
        return _this.UI['add-comment-box'].J.addClass("hide");
      };
      return this.UI['add-comment-confirm'].onclick = function() {
        var content, markid;
        content = _this.UI['add-comment-content'].value;
        if (!content || content.replace(" ", "").length === 0) {
        ...
        markid = _this.markData.markid;
        return _this.api.addCommentToMark(content, markid, function(res) {
          window.showMessage("Comment added successfully.");
          _this.UI['add-comment-content'].value = "";
          _this.UI['add-comment-box'].J.addClass("hide");
          if (!res.success) {
            window.showMessage(res.error_msg, "error");
          }
          return _this.app.initComments(function() {
            _this.rightSection.initComments();
            _this.rightSection.scrollToMarkComments(markid);
            return _this.app.scrollToRectMark(_this.markData);
          });
        });
      };
    };
  ...
  CommentsItemCommentLi = (function(_super) {
    __extends(CommentsItemCommentLi, _super);

    function CommentsItemCommentLi(tpl, data, commentsItem, api) {
      var _this = this;
      this.commentsItem = commentsItem;
      this.api = api;
      CommentsItemCommentLi.__super__.constructor.call(this, tpl);
      this.data = data;
      this.J.addClass("id-" + data.commentid);
      this.UI.content.J.html(data.content);
      this.textContent = this.UI.content.textContent;
      this.UI.content.J.text(this.textContent);
      this.UI.nickname.J.text(data.nickname);
      this.UI.date.J.text(Utils.parseTime(data.datetime * 1000, "Y-M-D"));
      this.UI['reply-num'].J.text(data.replynum);
      this.UI['vote-up-num'].J.text(data.praisenum);
      this.UI['vote-up'].onclick = function() {
        return _this.voteUpComment();
      };
      this.UI['vote-down'].onclick = function() {
      ...
      this.UI['fold-btn'].onclick = function(evt) {
        evt.stopPropagation();
        return _this.toggleUp();
      };
      this.UI['reply-btn'].onclick = function(evt) {
        evt.stopPropagation();
        return _this.emit("showSingleComment");
      };
      this.dom.onclick = function() {
        return _this.toggleDown();
      };
    }

    CommentsItemCommentLi.prototype.toggleDown = function() {
      this.J.removeClass("folded");
      this.UI.content.J.addClass("real-content-section");
      return this.UI.content.J.html(this.data.content);
    };

    CommentsItemCommentLi.prototype.toggleUp = function() {
      this.J.addClass("folded");
      this.UI.content.J.removeClass("real-content-section");
      return this.UI.content.J.text(this.textContent);
    };

    CommentsItemCommentLi.prototype.voteUpComment = function() {
      var _this = this;
      if (this.J.hasClass("folded")) {
        return false;
      }
      return this.api.voteupComment(this.data.commentid, function(res) {
        if (!res.success) {
          return console.error(res.error_msg);
        }
        _this.data.praisenum = res.praisenum;
        _this.UI['vote-up-num'].J.text(res.praisenum);
        return _this.commentsItem.updateVoteupNum(_this.data.commentid, res.praisenum);
      });
    };

    CommentsItemCommentLi.prototype.voteDownComment = function() {
      var _this = this;
      if (this.J.hasClass("folded")) {
        return false;
      }
      return this.api.votedownComment(this.data.commentid, function(res) {
        if (!res.success) {
          return console.error(res.error_msg);
        }
        _this.data.praisenum = res.praisenum;
